...
CYCODE const uint8 flatLine[TABLE_LENGTH] =
{
...
int32 ADCPhase, ADCDegrees, Pot, previous, current, wait;
...
void main(){
  ...
  //  #define DMA_1_DST_BASE (CYDEV_PERIPH_BASE)
    ...
    for(;;)
    {
        ...
        LCD_ClearDisplay();
        Pot = ADC_Read32();
        if(Pot > 0xFFF0) Pot = 0xFFFF; //Check Pot Value 
        if(Pot < 0x0005) Pot = 0x0000;
        
        ADCPhase = Pot*129 / 0xFFFF;
        if(current != ADCPhase) wait=ADCPhase;
        current = ADCPhase;

        LCD_PrintString("Pot:");
        LCD_PrintNumber(ADCPhase);
        ADCDegrees = Pot*361/ 0xFFFF;
        LCD_Position(1,0);
        LCD_PrintString("Deg:");
        LCD_PrintNumber(ADCDegrees);
       
        if(wait){
            if(VDAC8_1_Data == 128) /*| (VDAC8_1_Data == 122)| (VDAC8_1_Data == 134))*/{
                CyDmaTdSetAddress(DMA_1_TD[0], LO16((uint32)flatLine), LO16((uint32)VDAC8_1_Data_PTR));
            }wait--;
        }else if((VDAC8_1_Data == 128) | (VDAC8_1_Data == 122))CyDmaTdSetAddress(DMA_1_TD[0], LO16((uint32)sineTable+ADCPhase), LO16((uint32)VDAC8_1_Data_PTR));
        CyDelay(50);
    }
}
